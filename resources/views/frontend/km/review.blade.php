            ...
            @foreach($newests as $newest)
                <article
                    ...
                    <div class="post-list alignleft">
                        <a href="{{ url('tin-tuc/'.$newest->slug) }}" class="alignleft" aria-hidden="true"
                           style="background: url('{{ $newest->image }}') no-repeat center;">
                        </a>
                    </div>
                    <header class="entry-header">
                        <h2 class="entry-title" itemprop="headline">
                            <a href="{{  url('tin-tuc/'.$newest->slug) }}">{{ $newest->title }}</a>
                        </h2>
                        {{--<a href="{{  url('tin-tuc/'.$newest->slug) }}"><p style="word-wrap: break-word;"><span><i--}}
